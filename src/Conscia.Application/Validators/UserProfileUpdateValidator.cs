...
public class UserProfileUpdateValidator : AbstractValidator<UserProfileUpdateDto>
{
    ...
    public UserProfileUpdateValidator()
    {
        RuleFor(x => x)
            .Must(dto =>
                dto.DisplayName is not null ||
                dto.ProfilePictureKey is not null ||
        ...
        RuleFor(x => x.ProfilePictureKey)
            .Must(key =>
                key is null ||
                (key.StartsWith("profile-pictures/", StringComparison.Ordinal) &&
                 !key.Contains("..", StringComparison.Ordinal) &&
                 !key.Contains("://", StringComparison.Ordinal)))
            .WithMessage("Profile picture key must be generated by Conscia");
        ...
        RuleFor(x => x.PreferredCurrency)
            .Must(c => c is null || SupportedCurrencies.Codes.Contains(c.ToUpperInvariant()))
