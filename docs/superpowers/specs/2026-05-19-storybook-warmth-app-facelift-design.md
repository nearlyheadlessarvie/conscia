...
- **Libre Baskerville** for expressive headings and editorial moments
- **Nunito Sans** for body text, labels, controls, navigation, amounts, and dense UI

Libre Baskerville gives the app the Storybook Warmth signal: reflective, human, literary, and emotionally distinct from a generic finance dashboard. It should be used selectively so it feels special rather than noisy.

Nunito Sans should replace the current Poppins/Inter mix as the core product font. It keeps the interface friendly and readable while pairing better with Libre Baskerville than the current sharper body stack.

Recommended type roles:

- `displayLarge`, `displayMedium`, `headlineLarge`, `headlineMedium`, and `titleLarge`: Libre Baskerville
...
- replace inline `GoogleFonts.poppins` and `GoogleFonts.inter` calls with theme-driven typography or the approved Libre Baskerville/Nunito Sans stack
...
- Libre Baskerville and Nunito Sans are the dominant app-wide font system
